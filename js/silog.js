...
`;
                ...
                overlayAddToCartBtn.onclick = async function () {
                    ...
                    if (!authUser) {
                        console.warn('Firebase write blocked: add silog to cart at "users/{uid}/ordercart" requires an authenticated Firebase user.');
...
if (cartIcon) {
    ...
    setTimeout(() => {
        cartIcon.classList.remove('cart-grow-animate');
                        ...
                        .catch((error) => {
                            console.error(`Firebase write failed: add silog to cart at "users/${userUid}/ordercart"`, error);
