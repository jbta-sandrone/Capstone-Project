...
`;
                ...
                overlayAddToCartBtn.onclick = async function () {
                    ...
                    if (!authUser) {
                        console.warn('Firebase write blocked: add rice meal to cart at "users/{uid}/ordercart" requires an authenticated Firebase user.');
...
if (cartIcon) {
    ...
    setTimeout(() => {
        cartIcon.classList.remove('cart-grow-animate');
                        ...
                        .catch((error) => {
                            console.error(`Firebase write failed: add rice meal to cart at "users/${userUid}/ordercart"`, error);
